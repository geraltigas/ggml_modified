...
static void
ggml_vec_dot_f32(int n, float *restrict s, size_t bs, const float *restrict x, size_t bx, const float *restrict y,
                 size_t by, int nrc);

static void
ggml_vec_dot_f16(int n, float *restrict s, size_t bs, ggml_fp16_t *restrict x, size_t bx, ggml_fp16_t *restrict y,
                 size_t by, int nrc);
...
inline static void
ggml_vec_dot_f16_unroll(const int n, const int xs, float *restrict s, void *restrict xv, ggml_fp16_t *restrict y) {
...
inline static void
ggml_vec_mad_f32_unroll(const int n, const int xs, const int vs, float *restrict y, const float *restrict xv,
...
void
ggml_unravel_index(const struct ggml_tensor *tensor, int64_t i, int64_t *i0, int64_t *i1, int64_t *i2, int64_t *i3) {
...
static struct ggml_tensor *ggml_add_cast_impl(
    ...
    GGML_ASSERT(ggml_is_quantized(a->type) ||
                a->type == GGML_TYPE_F16); // currently only supported for quantized input and f16
...
struct ggml_tensor *ggml_conv_2d(
    ...
    struct ggml_tensor *im2col = ggml_im2col(ctx, a, b, s0, s1, p0, p1, d0, d1, true,
                                             GGML_TYPE_F16); // [N, OH, OW, IC * KH * KW]
...
struct ggml_tensor *ggml_ssm_conv(
    ...
    struct ggml_tensor *result = ggml_new_tensor_1d(ctx, GGML_TYPE_F32,
                                                    (d_inner * n_tokens) + (d_conv * d_inner * n_kv));
...
static void ggml_compute_forward_acc_f32(
    ...
    GGML_ASSERT(offset + (ne10 == 0 ? 0 : ne10 - 1) * nb0 + (ne11 == 0 ? 0 : ne11 - 1) * nb1 +
                (ne12 == 0 ? 0 : ne12 - 1) * nb2 + (ne13 == 0 ? 0 : ne13 - 1) * nb3 < ggml_nbytes(dst));
    GGML_ASSERT(offset + (ne10 == 0 ? 0 : ne10 - 1) * nb00 + (ne11 == 0 ? 0 : ne11 - 1) * nb01 +
                (ne12 == 0 ? 0 : ne12 - 1) * nb02 + (ne13 == 0 ? 0 : ne13 - 1) * nb03 < ggml_nbytes(src0));
                        ...
                        for (int k1 = 0; k1 < ne01; k1++) {
                            for (int i0 = 0; i0 < nr0; i0++) {
                                ggml_fp16_t *y = (ggml_fp16_t *) ((char *) dst->data + (i3 * ne03 + k3) * nb3 +
                                                                  (i2 * ne02 + k2) * nb2 + (i1 * ne01 + k1) * nb1 +
                                                                  (i0 * ne00) * nb0);
                                ggml_fp16_t *x = (ggml_fp16_t *) ((char *) src0->data + (k3) * nb03 + (k2) * nb02 +
                                                                  (k1) * nb01);
...
static void ggml_compute_forward_mul_mat_id(
    ...
    char *wdata_src1_end = (src1->type == vec_dot_type) ?
                           (char *) params->wdata :
                           (char *) params->wdata +
                           GGML_PAD(ggml_row_size(vec_dot_type, ggml_nelements(src1)), sizeof(int64_t));
    ...
    for (int64_t i3 = 0; i3 < ne3; i3++) {
        for (int64_t i2 = 0; i2 < ne2; i2++) {
            ...
            if (!is_glm && !is_neox) { // TODO: cache sin/cos for glm, neox
                ggml_rope_cache_init(p, freq_scale, corr_dims, ne0, ext_factor, attn_factor, cache, sin_sign,
                                     theta_scale);
            }
                ...
                if (is_glm) {
                    ...
                    for (int64_t ic = 0; ic < ne0; ic += 2) {
                        if (ic < n_dims) {
                            ...
                            const float *const src = (float *) ((char *) src0->data + i3 * nb03 + i2 * nb02 +
                                                                i1 * nb01 + i0 * nb00);
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {
                ...
                if (is_glm) {
                    ...
                    for (int64_t i0 = 0; i0 < ne0 / 4; i0++) {
                        ...
                        const ggml_fp16_t *const src = (ggml_fp16_t *) ((char *) src0->data + i3 * nb03 + i2 * nb02 +
                                                                        i1 * nb01 + i0 * nb00);
                    ...
                    for (int64_t ic = 0; ic < ne0; ic += 2) {
                        if (ic < n_dims) {
                            ...
                            const ggml_fp16_t *const src = (ggml_fp16_t *) ((char *) src0->data + i3 * nb03 +
                                                                            i2 * nb02 + i1 * nb01 + i0 * nb00);
                            ggml_fp16_t *dst_data = (ggml_fp16_t *) ((char *) dst->data + i3 * nb3 + i2 * nb2 +
                                                                     i1 * nb1 + i0 * nb0);
                        ...
                        for (int64_t ikh = 0; ikh < KH; ikh++) {  // 1
                            for (int64_t ikw = 0; ikw < KW; ikw++) {
                                ...
                                if (iih < 0 || iih >= IH || iiw < 0 || iiw >= IW) {
                                    ...
                                    dst_data[iic * (KH * KW) + ikh * KW + ikw] = GGML_FP32_TO_FP16(
                                            src_data[iih * IW + iiw]);
                                }
                            }
                        }
        ...
        for (int64_t i2 = ith; i2 < ne2; i2 += nth) {
            ...
            for (int64_t i1 = 0; i1 < ne1; i1++) {
                ...
                for (int64_t i0 = 0; i0 < ne0; i0++) {
                    ...
                    const float *x = (float *) ((char *) src0->data + i00 * nb00 + i01 * nb01 + i02 * nb02 +
                                                i03 * nb03);
                    float *y = (float *) ((char *) dst->data + i0 * nb0 + i1 * nb1 + i2 * nb2 + i3 * nb3);
        ...
        for (int64_t i1 = ith; i1 < ne1; i1 += nth) {
            for (int64_t i0 = 0; i0 < ne0; ++i0) {
                for (int64_t i3 = 0; i3 < ne3; ++i3) {
                    ...
                    const float *src_ptr = (const float *) ((char *) src0->data + i3 * nb03 + i2 * nb02 + i1 * nb01 +
                                                            i0 * nb00);
...
static void ggml_compute_forward_ssm_conv_f32(
    ...
    if (n_kv > 1) {
        ...
        for (int i3 = 0; i3 < n_kv; ++i3) {
            ...
            float *s = (float *) ((char *) dst->data + ir0 * (src2->nb[1]) + i3 * (src2->nb[2]) +
                                  nr * n_t * sizeof(float));
    ...
    for (int i2 = 0; i2 < n_t; ++i2) {
        ...
        float *x = (float *) ((char *) dst->data + ir0 * sizeof(float) +
                              i2 * (nr * sizeof(float))); // {d_inner, n_tokens}
        float *s = (float *) ((char *) dst->data + ir0 * (src2->nb[1]) + sq[0] * (src2->nb[2]) +
                              nr * n_t * sizeof(float)); // {d_conv, d_inner, n_kv}
        ...
        if (i2 == 0) {
            s0 = (float *) ((char *) src0->data + ir0 * (src0->nb[1]) +
                            sq[0] * (src0->nb[2])); // {d_conv - 1, d_inner, n_kv}
            ne0s0 = src0->ne[0];
        } else {
...
static void ggml_compute_forward_ssm_scan_f32(
    ...
    for (int i2 = 0; i2 < n_t; ++i2) {
        ...
        float *s = (float *) ((char *) dst->data + ir0 * (src0->nb[1]) + sq[0] * (src0->nb[2]) +
                              src1->nb[2]); // {d_state, d_inner, n_kv}
        float *s0;
        ...
        if (i2 == 0) {
            s0 = (float *) ((char *) src0->data + ir0 * (src0->nb[1]) +
                            sq[0] * (src0->nb[2])); // {d_state, d_inner, n_kv}
        } else {
...
ggml_acc_or_set(struct ggml_context *ctx, struct ggml_tensor *a, struct ggml_tensor *b, size_t nb1, size_t nb2,
                size_t nb3, size_t offset, struct ggml_hash_set zero_table) {
    if (ggml_hash_contains(zero_table, a)) {
...
static void
ggml_compute_backward(struct ggml_context *ctx, struct ggml_tensor *tensor, struct ggml_hash_set zero_table) {
...
static void
ggml_graph_compute_thread_sync_task(int *task_phase, struct ggml_compute_state *state, const bool do_yield) {
    // wait for other threads to finish
...
struct ggml_cgraph *
ggml_graph_import(const char *fname, struct ggml_context **ctx_data, struct ggml_context **ctx_eval) {
    ...
    // populate result
    {
        ...
        // create the data context
        {
            const size_t overhead =
                    (n_leafs + n_nodes) * ggml_tensor_overhead() + ggml_graph_overhead_custom(graph_size, false);
...
static void
ggml_graph_dump_dot_leaf_edge(FILE *fp, struct ggml_tensor *node, struct ggml_tensor *parent, const char *label) {
...
static enum ggml_opt_result ggml_opt_lbfgs(
    ...
    while (true) {
        ...
        ls = linesearch_backtracking(&params, nx, x, &fx, g, d, step, xp, f, gb, &cplan, np, ps, &cancel, callback,
                                     callback_data);
        if (cancel) {
            return GGML_OPT_RESULT_CANCEL;
        }
    ...
    if (opt->ctx == NULL) {
        ...
        if (opt->params.type == GGML_OPT_TYPE_ADAM) {
            ctx_opt_params.mem_size =
                    GGML_MEM_ALIGN * 3 + ggml_tensor_overhead() * 3 + ggml_type_size(GGML_TYPE_F32) * nx * 3;
            if (opt->params.past > 0) {
                ctx_opt_params.mem_size +=
                        GGML_MEM_ALIGN + ggml_tensor_overhead() + ggml_type_size(GGML_TYPE_F32) * opt->params.past;
            }
        } else if (opt->params.type == GGML_OPT_TYPE_LBFGS) {
            ctx_opt_params.mem_size = GGML_MEM_ALIGN * 9 + ggml_tensor_overhead() * 9 + ggml_type_size(GGML_TYPE_F32) *
                                                                                        (nx * 5 +
                                                                                         opt->params.lbfgs.m * 2 +
    ...
    // check the magic before making allocations
    {
        ...
        for (uint32_t i = 0; i < sizeof(magic); i++) {
            if (magic[i] != GGUF_MAGIC[i]) {
                fprintf(stderr, "%s: invalid magic characters '%c%c%c%c'\n", __func__, magic[0], magic[1], magic[2],
                        magic[3]);
                fclose(file);
                return NULL;
            }
    ...
    // compute the total size of the data section, taking into account the alignment
    {
        ...
        for (uint64_t i = 0; i < ctx->header.n_tensors; ++i) {
            ...
            if (ne % ggml_blck_size(info->type) != 0) {
                fprintf(stderr,
                        "%s: tensor '%s' of type %d (%s) number of elements (%" PRId64 ") is not a multiple of block size (%d)\n",
                        __func__, info->name.data, (int) info->type, ggml_type_name(info->type), ne,
